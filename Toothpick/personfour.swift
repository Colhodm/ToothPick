...
class personfour: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        personFourText.addTarget(self, action: #selector(searchRecords(textField:)), for: .editingChanged)


        // Do any additional setup after loading the view.
    }
    @objc func searchRecords(textField:UITextField){
        if personFourText.text!.count >= 4 {
            if personFourText.text!.count == 4{
                placeAutocomplete()
            }
            if self.querylen <= personFourText.text!.count{
                filterArray()
            } else {
                placeAutocomplete()
    ...
    func filterArray(){
        ...
        for restaurant in myArray{
            if restaurant.contains(personFourText.text!){
                temp.append(restaurant)
            }
        }
        self.myArray = temp
        self.querylen = personFourText.text!.count
        
    }
    func placeAutocomplete() {
        let filter = GMSAutocompleteFilter()
        filter.type = .establishment
        placesClient.autocompleteQuery(personFourText.text!, bounds: nil, filter: filter, callback: {(results, error) -> Void in
            if let error = error {
    ...
    if numPeople! > 4  {
            ...
            /*Use the Identifier you given in story Board*/
            self.performSegue(withIdentifier: "personfourtoend", sender: self)
        }
    }
...
extension personfour: UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if cell.textLabel?.text?.count == 0{
            personFourText.text = "Sorry, no matches were found please enter a new query"
            myOptions.isHidden = true
            return
        } else {
            personFourText.text = cell.textLabel?.text
...
extension personfour: CLLocationManagerDelegate {
